...
public class UMConfigure {
    ...
    private static void deleteModuleTag(BS_TYPE bs_type, String str) {
        ...
        try {
            ...
            TagHelper.deleteModuleTag(str2 + JavaConstant.Dynamic.DEFAULT_NAME + str);
        } catch (Throwable unused) {
        }
    }
    ...
    public static void getOaid(Context context, OnGetOaidListener onGetOaidListener) {
        ...
        if (shouldCollectOaid) {
            new Thread(new Runnable() { // from class: com.umeng.commonsdk.UMConfigure.4
                ...
                public void run() {
                    String strB = bf.b(applicationContext);
    ...
    private static void preInitInternal(Context context, String str, String str2) {
        ...
        if (SdkVersion.MINI_VERSION.equals("0")) {
            shouldOutputRT = true;
        }
        if (TextUtils.isEmpty(str)) {
    ...
    public static void setLogEnabled(boolean z) {
        try {
            ...
            if (debugLog) {
                dts.f("set log enabled e is ", TAG, th);
            }
        }
    }
    ...
    private static void setModuleTag(BS_TYPE bs_type, String str, String str2) {
        try {
            ...
            if (!TextUtils.isEmpty(str) && !TextUtils.isEmpty(str2)) {
                ...
                TagHelper.setModuleTag(str3 + JavaConstant.Dynamic.DEFAULT_NAME + str, str2);
                return;
            }
    ...
    public static void init(Context context, String str, String str2, int i, String str3) {
        ...
        try {
            try {
                if (debugLog) {
                    Log.i(TAG, "common version is 9.8.8");
                    Log.i(TAG, "common type is " + SdkVersion.SDK_TYPE);
                }
            } catch (Exception e) {
                if (debugLog) {
                    dts.e("init e is ", e, TAG);
                }
            }
        } catch (Throwable th) {
            if (debugLog) {
                dts.f("init e is ", TAG, th);
            }
        }
        if (context == null) {
            if (debugLog) {
