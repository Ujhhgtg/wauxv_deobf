...
public abstract class drm {
    ...
    public static void h(View view, Context context) {
        TypedArray typedArrayObtainStyledAttributes = context.obtainStyledAttributes(cyf.j);
        try {
    ...
    public static int i(Context context, int i) {
        ColorStateList colorStateListK = k(context, i);
        if (colorStateListK != null && colorStateListK.isStateful()) {
            return colorStateListK.getColorForState(b, colorStateListK.getDefaultColor());
        }
        ...
        return abi.f(j(context, i), Math.round(Color.alpha(r4) * f2));
    }
