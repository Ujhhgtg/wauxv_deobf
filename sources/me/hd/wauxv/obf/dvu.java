package me.hd.wauxv.obf;

/* JADX INFO: compiled from: r8-map-id-b9de5da7d0413052737328a4e696e1bcc3145db8f6a41e1e318485e124198cd6 */
/* JADX INFO: loaded from: classes.dex */
public final class dvu {
    public static final dvt Companion = new dvt();
    public final Integer a;
    public final Integer b;
    public final Integer c;
    public final Integer d;

    public /* synthetic */ dvu(int i, Integer num, Integer num2, Integer num3, Integer num4) {
        if (15 != (i & 15)) {
            ResourcesCompat.ah(i, 15, dvs.b.getDescriptor());
            throw null;
        }
        this.a = num;
        this.b = num2;
        this.c = num3;
        this.d = num4;
    }

    public final boolean equals(Object obj) {
        if (this == obj) {
            return true;
        }
        if (!(obj instanceof dvu)) {
            return false;
        }
        dvu dvuVar = (dvu) obj;
        return nullSafeIsEqual(this.a, dvuVar.a) && nullSafeIsEqual(this.b, dvuVar.b)
                && nullSafeIsEqual(this.c, dvuVar.c) && nullSafeIsEqual(this.d, dvuVar.d);
    }

    public final int hashCode() {
        Integer num = this.a;
        int iHashCode = (num == null ? 0 : num.hashCode()) * 31;
        Integer num2 = this.b;
        int iHashCode2 = (iHashCode + (num2 == null ? 0 : num2.hashCode())) * 31;
        Integer num3 = this.c;
        int iHashCode3 = (iHashCode2 + (num3 == null ? 0 : num3.hashCode())) * 31;
        Integer num4 = this.d;
        return iHashCode3 + (num4 != null ? num4.hashCode() : 0);
    }

    public final String toString() {
        StringBuilder sb = new StringBuilder();
        sb.append("be5Proto(ios_sdk_version_min=" /* "be5Proto(ios_sdk_version_min=" /* "be5Proto(ios_sdk_version_min=" /* cnb.z(-168543106628394L)  */);
        bjs.w(sb, this.a, -174513111169834L);
        bjs.w(sb, this.b, -174397147052842L);
        bjs.w(sb, this.c, -174796579011370L);
        return concat(sb, this.d, ')');
    }
}
