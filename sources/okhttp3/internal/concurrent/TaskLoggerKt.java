...
public final class TaskLoggerKt {
    ...
    public static final void log(Task task, TaskQueue taskQueue, String str) {
        TaskRunner.Companion.getLogger().fine(taskQueue.getName$okhttp() + ' '
    ...
    public static final <T> T logElapsed(Task task, TaskQueue taskQueue, bfu bfuVar) {
        ...
        try {
            ...
            if (zIsLoggable) {
                log(task, taskQueue, "failed a run in "
                        + formatDuration(taskQueue.getTaskRunner$okhttp().getBackend().nanoTime() - jNanoTime));
            }
        }
    }
